...
class RnnDecoder(Decoder):
    ...
    def forward(self, trip, valid_len, encode):
        ...
        x = trip
        init_h = rearrange(self.init_linear(encode), 'B (D H) -> D B H', D=self.num_layers)
        ...
        tgt = pack_padded_sequence(tgt, lengths=valid_len, batch_first=True, enforce_sorted=False)
    ...
    def get_pre_label(x, valid_len, col, out, predictor, discrete):
        pre = predictor(out)
        label = RnnDecoder.get_pack_data(x[:, :, col], valid_len)
        if discrete:
            label = label.long()
            return pre.argmax(-1).unsqueeze(-1).long().detach().cpu().numpy(), \
                label.unsqueeze(-1).long().detach().cpu().numpy(), \
                F.cross_entropy(pre, label)
        else:
            return pre.detach().cpu().numpy(), \
                label.detach().cpu().numpy(), \
                F.mse_loss(pre, label)
